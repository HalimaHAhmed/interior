...
const Care = () => {
  return <div id="care" className="px-10 max-w-[1490px] mx-auto mb-[160px]">

    <FadeIn delay={0.2} direction="down">
      <h5 className="text-5xl lg:text-[64px] font-medium text-fontBlack mb-6 text-center">{careTitle}</h5>
    </FadeIn>

    <FadeIn delay={0.2} direction="down">

      <h5 className="text-[#4F4F4F] text-lg xs:text-xl mb-12">{careSubtitle}</h5>
    </FadeIn>
      ...
      {careList.map((item,i)=>(
        <FadeIn key={i} delay={(i+1) *0.2} direction="left">
          <div className="flex flex-col xs:flex-row gap-6 items-center xs:items-start">
            ...
            <img  src={item.img} alt={item.title} className="h-[88px] w-[68px]"/>
